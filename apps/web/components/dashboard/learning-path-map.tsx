...
export function LearningPathMap({ onNavigate }: LearningPathMapProps) {
  ...
  return (
    <div className="bg-slate-950 border border-slate-900 rounded-3xl p-8 relative overflow-hidden">
      <div className="flex justify-between items-center mb-10">
        <SectionTitle icon={Map} iconClassName="text-indigo-400">
          Learning Path Map
        </SectionTitle>
        <button
              ...
              <motion.div
                ...
                className={`w-16 h-16 rounded-2xl border-2 flex items-center justify-center transition-all duration-300 shadow-xl ${
                  node.status === "completed"
                    ...
                    : node.status === "active"
                      ? "bg-violet-600/10 border-violet-500 text-violet-400 shadow-violet-500/30"
              ...
              <div className="text-center">
                <p
                  className={`text-sm font-bold ${
                    node.status === "locked" ? "text-slate-600" : "text-white"
                  }`}
                >
                  {node.label}
                </p>
